...
/** Actor responsible for managing a worker node.
  *
  * It establishes the connection to the seed node,
  * then connects to the master, 
  * then spawns multiple `Containment`s, in which it can run 
  * potentially instable code.
  */
class Worker(numContainments: Int) 
extends Actor 
with ActorLogging {
