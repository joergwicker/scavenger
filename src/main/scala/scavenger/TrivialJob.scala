...
import scavenger.util.Instance
  ...
  sealed trait TrivialJob[+X] {
    ...
    private[scavenger] def identifier: scavenger.Identifier
  ...
  (jobs: CC[TrivialJob[X]]) extends TrivialJob[CC[X]] {
    ...
    def eval(implicit ctx: TrivialContext): Future[Value[CC[X]]] = {
      ...
      val fvCbf = genProdCbf[CC, Future[Value[X]]]
      val vCbf = genProdCbf[CC, Value[X]]
    ...
    protected[scavenger] def mapChildren
      ...
      val fjCbf = genProdCbf[CC, Future[TrivialJob[X]]]
      val jCbf = genProdCbf[CC, TrivialJob[X]]
